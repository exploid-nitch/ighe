...
{O} ╭ 𝐿𝑜𝑔𝑖𝑛 𝐵𝑦 𝑁𝑖𝑡𝑐ℎ 🐊
  {O}╞{N}
  {O}╞{N} FulNama: {O}{nama}{C}
  {O}╞{N} Username: {O}{user}{C}
  {O}╞{N} Password: {O}{pw}{C}
  {O}╞{N} Pengikut: {O}{pengikut}{C}
  {O}╞{N} Mengikuti: {O}{mengikut}{C}
  {O}╞{N} Postingan: {O}{postingan}{C}
  {O}╞{N}
  {O}╰  𝐽𝑔𝑛 𝐿𝑢𝑝𝑎 𝐶𝑖𝑝𝑟𝑎𝑡 𝑀𝑒𝑘 🗿
  """)
					...
					nama,pengikut,mengikut,postingan=self.APIinfo(user)
					print(f"""
{M} ╭{N} 𝑴𝒂𝒎𝒑𝒖𝒔 𝑪𝒆𝒌 𝑷𝒐𝒊𝒏𝒕  🗿
  {M}╞{N}
  {M}╞{N} FulNama: {M}{nama}{C}
  {M}╞{N} Username: {M}{user}{C}
  {M}╞{N} Password: {M}{pw}{C}
  {M}╞{N} Pengikut: {M}{pengikut}{C}
  {M}╞{N} Mengikuti: {M}{mengikut}{C}
  {M}╞{N} Postingan: {M}{postingan}{C}
  {M}╞{N}
  {M}╰{N} 𝑴𝒂𝒎𝒑𝒖𝒔 𝑪𝒆𝒌 𝑷𝒐𝒊𝒏𝒕  🗿
  	""")
...
def lisensi():
	...
	res=ses.get('https://api.cryptolens.io/api/key/Activate?token=WyIxODc3NjYxMiIsIjAraVZQNUZXdmdHTXJWWlhubkxYTVprdC9TczR1QjI0c2dlSi9FMkoiXQ==&ProductId=15246&Key='+lisensikuni[0]).json()
